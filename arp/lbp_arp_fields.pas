...
Classes that know how to read their data from and write to a ARP packet
...
type
   ARPFieldException = class( lbp_exception);
...
procedure tFixedOpCode.Read( var Buffer: tNetBuffer;
   ...
   begin
      ...
      if ((Value <> BootRequestOp) and (Value <> BootReplyOp)) then begin
         raise ARPFieldException.Create( 'Invalid bootp opcode!');
      end;
   end; // Read()
...
procedure tFixedHWLength.Read( var Buffer: tNetBuffer;
   ...
   begin
      ...
      if( Value > 16) then begin
         raise ARPFieldException.Create(
...
constructor tARPOpCode.Create();
   begin
      inherited Create( 53, 'ARP Op-Code', bootpRequest);
   end; // Create()
...
// ************************************************************************
 ...
 function tARPOpCode.GetStrValue(): string;
    begin
       case Value of
          bootpRequest:   GetStrValue:= 'BootP Request';
          arpDiscover:   GetStrValue:= 'ARP Discover';
          arpOffer:      GetStrValue:= 'ARP Offer';
          arpRequest:    GetStrValue:= 'ARP Request';
          arpDecline:    GetStrValue:= 'ARP Decline';
          arpAck:        GetStrValue:= 'ARP Acknowledge';
          arpNAck:       GetStrValue:= 'ARP Negative Acknowledge';
          arpRelease:    GetStrValue:= 'ARP Release';
          arpInform:     GetStrValue:= 'ARP Inform';
          else            GetStrValue:= 'Unknown operation';
       end; // case
    end; // GetStrValue()
...
constructor tVarLease.Create();
   begin
      inherited Create( 51, 'ARP Lease', DefaultLease);
   end; // Create()
   ...
   begin
      ...
      for iStr:= 1 to TempLength do begin
         if( TempValue[ iStr] in ['0'..'9']) then begin
            ...
            raise  ARPFieldException.Create( 'VarClientID.SetStrValue():  ' +
                                   'Invalid character in input string!');
         end;
         if( HighNibble) then begin
            Value[ iByte]:= Temp SHL 4;
